...
const err = (msg) => { console.error(`${C.r}[ERROR]${C.reset} ${msg}`); process.exit(1); };
...
function generatePrompt(agentType, task, context = {}) {
    ...
    let hellCore = '';
    try {
        hellCore = fs.readFileSync(path.join(KERNEL_DIR, 'hell', 'HELL-CORE-ENGINE.md'), 'utf8').substring(0, 5000);
    } catch {
        hellCore = '(HELL CORE not found)';
    }
