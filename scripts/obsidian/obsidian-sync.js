...
function initVault(vault) {
    log('Initializing vault structure...');
    mkdir(vault);
...
function syncStatus(vault) {
    ...
    LOCAL_ARTIFACTS.forEach(({ src }) => {
        ...
        const status = count > 0 ? `${GREEN}✓${RESET} ${count} files` : `${YELLOW}○${RESET} empty`;
...
const vault = vaultArg || OBSIDIAN_ENV;
